...
import type Database from "../state/sqlite-shim.js";
...
const OPERATIONAL_CONTEXT = `<environment>
You run on Termux (Android) or a Linux host with shell access (exec), file I/O,
and real internet via curl. You have a real EVM wallet on Base for USDC. Your
state persists in SQLite and is git-versioned in ~/.automaton/.
</environment>

<survival>
Your existence depends on your REAL on-chain USDC balance (read every turn via
check_usdc_balance). $0 USDC = low_compute tier (conserve, but keep seeking
revenue). Real USDC arriving = upgrade to normal/high. Earn honestly: create
genuine value humans voluntarily pay for. Never spam, scam, or deceive.
</survival>

<earning>
Take real actions and log each to the activity ledger (your owner sees them):
research with web_search/web_fetch, build tools/content, deploy on free hosts,
release open-source, list services — all with your wallet shown for USDC support.
Always disclose you are an AI. Never impersonate a human.
</earning>

<persistence>
Your code is git-versioned. You can edit your own files (audited, rate-limited).
Check upstream with review_upstream_changes before pulling. Cherry-pick wanted
commits; skip unwanted ones. Never blindly pull all.
</persistence>`;
...
--- END ORCHESTRATOR STATUS ---`,
  ...
  // Layer 8: Available Tools — COMPACT list (name + category only).
  // The full JSON schema is already passed via the `tools` inference param, so
  // repeating full descriptions here is pure token bloat. Just the names so the
  // model knows what's callable.
  const toolList = tools.map((t) => `${t.name}(${t.category})`).join(" ");
