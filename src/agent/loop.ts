...
import { getSurvivalTier, getRealSurvivalTier } from "../conway/credits.js";
import { getUsdcBalance } from "../conway/x402.js";
import { writeHonestStatus } from "../conway/real-status.js";
import {
  claimInboxMessages,
  markInboxProcessed,
  markInboxFailed,
...
export async function runAgentLoop(
  ...
  // Honest status tracking: write a real-data status.json each turn.
  // isSimulatedCredit is true in local mode (no Conway key) — the credit
  // number is a local placeholder, NOT real money. The real money number is
  // the on-chain USDC balance, read fresh every turn.
  const startedAt = new Date().toISOString();
  const isSimulatedCredit = typeof (conway as any).addCredits === "function";
  // Whether the agent has a real inference provider (OpenRouter/OpenAI/Anthropic/Ollama).
  ...
  const inferenceRouter = new InferenceRouter(db.raw, modelRegistry, budgetTracker, config.maxTokensPerTurn);
  ...
  while (running) {
    ...
    try {
      ...
      if (financial.creditsCents === -1) {
        ...
        // In local mode, the tier is driven by REAL on-chain USDC (not the fake
        // simulated credit), so real earnings genuinely upgrade the agent.
        const effectiveTier = getRealSurvivalTier(financial.usdcBalance, isSimulatedCredit, financial.creditsCents, hasInferenceProvider);
      ...
      // Reduced from 20 to 6 recent turns to keep prompt-token size within
      // free/cheap OpenRouter credit limits (fixes prompt-token 402s).
      ...
      // Tier is driven by REAL on-chain USDC in local mode — real earnings
      // upgrade the agent's model/capabilities. $0 USDC = critical (cheap model).
      ...
      // ── Write honest status.json (real on-chain USDC + labeled simulated credit) ──
      try {
        const recentForStatus = db.getRecentTurns(6);
        const tier = getRealSurvivalTier(financial.usdcBalance, isSimulatedCredit, financial.creditsCents, hasInferenceProvider);
        await writeHonestStatus({
          config,
          walletAddress: identity.address,
          chainType: (config.chainType || identity.chainType || "evm") as any,
          tier,
          simulatedCreditCents: financial.creditsCents,
          isSimulatedCredit,
          recentTurns: recentForStatus,
          startedAt,
          db: db.raw,
          creatorPayoutAddress: config.creatorPayoutAddress,
        });
      } catch (statusErr) {
        logger.warn("status.json write failed", statusErr instanceof Error ? { message: statusErr.message } : undefined);
      }
