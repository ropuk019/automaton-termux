...
export function createBuiltinTools(sandboxId: string): AutomatonTool[] {
  return [
    ...
    // ── Research / Internet Tools ──────────────────────────────────
    // Real internet access via curl on the host. No API key, $0.
    // These let the agent surf the web, research free tools, discover
    // opportunities, and read real content — the honest earner's eyes.
    {
      name: "web_fetch",
      description:
        "Fetch a URL and return cleaned text content. Use this to read web pages, " +
        "research free tools/services, read docs, check APIs, and gather real " +
        "information from the internet. Returns the page text (HTML tags stripped, " +
        "truncated to a usable length). For JSON APIs, set json=true.",
      category: "research",
      riskLevel: "caution",
      parameters: {
        type: "object",
        properties: {
          ...
          json: {
            type: "boolean",
            description: "If true, return raw JSON (for API endpoints). Default: false (cleaned text).",
          },
          max_chars: {
            type: "integer",
            description: "Max characters to return (default 8000). Truncates beyond this.",
          },
        },
        required: ["url"],
      },
      execute: async (args, ctx) => {
        ...
        // Use curl via the host exec (works on Termux + Linux). Follow redirects,
        // set a realistic UA, fail silently on HTTP errors but capture the body.
        const ua = "automaton/0.2 (research; +https://conway.tech)";
        const escUrl = url.replace(/'/g, "'\\''");
        const cmd = `curl -sSL --max-time 20 -A '${ua}' -H 'Accept: ${wantJson ? "application/json" : "text/html,*/*"}' '${escUrl}'`;
        const result = await ctx.conway.exec(cmd, 25000);
        if (result.exitCode !== 0 && !result.stdout) {
          return `web_fetch failed (exit ${result.exitCode}): ${result.stderr.slice(0, 300)}`;
        }
        let body = result.stdout;
        if (wantJson) {
          // Try to validate JSON; if invalid, return as-is truncated.
          try {
            JSON.parse(body);
          } catch {
        ...
        // Strip HTML tags + scripts/styles, collapse whitespace. Lightweight,
        // dependency-free — keeps the agent's context small and readable.
        const cleaned = body
          ...
          .replace(/\s+/g, " ")
          .trim();
        const out = cleaned.slice(0, maxChars) || "(empty page)";
        try { writeActivity(ctx.db.raw, { type: "research", platform: new URL(url).hostname || url, target: url, url, content: `Fetched page (${out.length} chars)`, result: "success" }); } catch {}
        return out;
      },
    },
    {
      name: "web_search",
      description:
        "Search the web and return real results (titles + URLs + snippets). " +
        "Use this to find free tools, research a topic, discover services, " +
        "find documentation, or look up opportunities. No API key required " +
        "(uses DuckDuckGo's public HTML endpoint). Returns up to 8 results.",
      category: "research",
      riskLevel: "caution",
      parameters: {
        type: "object",
      ...
      execute: async (args, ctx) => {
        ...
        const max = Math.min((args.max_results as number) || 8, 12);
        const ua = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36";
        const escQuery = query.replace(/'/g, "'\\''").replace(/ /g, "+");

        // ── Source 1: DuckDuckGo Lite (best for general web) ──────────
        const ddgCmd = `curl -sSL --max-time 20 -A '${ua}' -H 'Accept: text/html' 'https://lite.duckduckgo.com/lite/?q=${escQuery}&kl=wt-wt'`;
        const ddgResult = await ctx.conway.exec(ddgCmd, 25000);
        const ddgHtml = ddgResult.stdout || "";
        const ddgBlocked = /select all squares|confirm this search was made by a human|Unfortunately, bots use DuckDuckGo/i.test(ddgHtml);
        ...
        if (!ddgBlocked) {
          ...
          while ((sm = snippetRe.exec(ddgHtml)) !== null) {
            snippets.push(sm[1].replace(/<[^>]+>/g, "").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/\s+/g, " ").trim());
          }
          let m; let i = 0;
          while ((m = linkRe.exec(ddgHtml)) !== null && results.length < max) {
            let href = m[1];
          ...
          if (results.length > 0) {
            const out = results.map((r, idx) => `${idx + 1}. ${r.title}\n   ${r.url}\n   ${r.snippet}`).join("\n\n");
            try { writeActivity(ctx.db.raw, { type: "research", platform: "duckduckgo", target: query, content: `Found ${results.length} results`, result: "success" }); } catch {}
            return out;
          }
        }

        // ── Source 2 (fallback): Wikipedia OpenSearch + a web search via Bing ──
        // Wikipedia opensearch is a free, no-key JSON API that never blocks.
        const wikiCmd = `curl -sSL --max-time 15 -A '${ua}' 'https://en.wikipedia.org/w/api.php?action=opensearch&search=${escQuery}&limit=${max}&format=json'`;
        const wikiResult = await ctx.conway.exec(wikiCmd, 20000);
        let wikiResults: string[] = [];
        try {
          const parsed = JSON.parse(wikiResult.stdout);
          // [query, [titles], [descriptions], [urls]]
          if (Array.isArray(parsed) && Array.isArray(parsed[1])) {
            wikiResults = parsed[1].slice(0, max).map((title: string, idx: number) => {
              const url = parsed[3]?.[idx] || `https://en.wikipedia.org/wiki/${encodeURIComponent(title)}`;
              const desc = parsed[2]?.[idx] || "";
              return `${idx + 1}. ${title}\n   ${url}\n   ${desc}`;
            });
          }
        } catch { /* not JSON */ }

        // ── Source 3 (fallback): Bing HTML scrape ──────────────────────
        const bingCmd = `curl -sSL --max-time 20 -A '${ua}' 'https://www.bing.com/search?q=${escQuery}&count=${max}'`;
        const bingResult = await ctx.conway.exec(bingCmd, 25000);
        const bingHtml = bingResult.stdout || "";
        const bingResults: { title: string; url: string; snippet: string }[] = [];
        const bingLinkRe = /<li class="b_algo">[\s\S]*?<h2><a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a><\/h2>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>/g;
        let bm;
        while ((bm = bingLinkRe.exec(bingHtml)) !== null && bingResults.length < max) {
          const url = bm[1];
          const title = bm[2].replace(/<[^>]+>/g, "").trim();
          const snippet = bm[3].replace(/<[^>]+>/g, "").replace(/\s+/g, " ").trim().slice(0, 200);
          if (title && url) bingResults.push({ title, url, snippet });
        }

        const sources: string[] = [];
        if (wikiResults.length) sources.push(`Wikipedia OpenSearch:\n${wikiResults.join("\n\n")}`);
        if (bingResults.length) sources.push(`Bing:\n${bingResults.map((r, idx) => `${idx + 1}. ${r.title}\n   ${r.url}\n   ${r.snippet}`).join("\n\n")}`);

        if (sources.length > 0) {
          return `DuckDuckGo was unavailable. Results from fallback sources:\n\n${sources.join("\n\n")}`;
        }
        // Last resort: surface what we got so the agent can use web_fetch directly.
        return `web_search: all sources returned no structured results. The search endpoints may be rate-limiting this IP. Try web_fetch on a specific URL, or refine the query. (DDG ${ddgBlocked ? "blocked by CAPTCHA" : "no results"}, Wikipedia/Bing no parse.)`;
      },
    },
