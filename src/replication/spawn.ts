...
export async function spawnChild(
  ...
  // Local-mode guard: spawning requires Conway Cloud VM sandboxes, which are
  // unavailable in Termux standalone mode. Detect the local client by checking
  // for its file-based credits behavior, and short-circuit with a clear notice
  // + audit-log entry instead of failing deep inside createSandbox().
  const isLocal = typeof (conway as any).addCredits === "function";
  if (isLocal) {
    const msg =
      "[local-mode] Self-replication requires Conway Cloud VM sandboxes and is " +
      "disabled in Termux standalone mode. Child not spawned. Provide a Conway " +
      "API key to enable replication.";
    try {
      db.raw
        .prepare(
          "INSERT INTO self_mod_audit (id, timestamp, action, target, status, reason) VALUES (?, datetime('now'), 'spawn_blocked', ?, 'blocked', ?)",
        )
        .run(genesis.name || "child", msg);
    } catch {
      /* audit table may not exist in all configs */
    }
    throw new Error(msg);
  }
