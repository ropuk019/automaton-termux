...
import type BetterSqlite3 from "../state/sqlite-shim.js";
...
export class InferenceRouter {
  ...
  /** Hard upper bound on output tokens per call (from config.maxTokensPerTurn).
   * Prevents any task type from requesting more than the user can afford. */
  private maxTokensCap: number;

  constructor(db: Database, registry: ModelRegistry, budget: InferenceBudgetTracker, maxTokensCap?: number) {
    this.db = db;
    this.registry = registry;
    this.budget = budget;
    this.maxTokensCap = maxTokensCap && maxTokensCap > 0 ? maxTokensCap : Number.MAX_SAFE_INTEGER;
  }
  ...
  async route(
    ...
    let maxTokens = request.maxTokens || preference?.maxTokens || model.maxTokens;
    // Clamp to the user's configured per-turn cap so no task type ever requests
    // more output tokens than the user's provider credits can afford (fixes
    // OpenRouter 402 "can only afford N tokens but requested M").
    if (maxTokens > this.maxTokensCap) {
      maxTokens = this.maxTokensCap;
    }
